...
- Added a per-slot empty socket image, so a single item can mix a battery bay, a rune notch, and a plain socket instead of only tinting the same frame. It is set in Socket Slot Settings (field or click on the slot preview), in the Add Socket prompt, and through `slotConfig.frameImg` in the public API, with a `Default` button that restores the socket artwork shipped with the module. The custom image is used by the Sockets tab, Tidy views, actor badges, socket descriptions, and the `sc-more-activities` pickers.
...
- Redesigned the Socket Slot Settings window as a master–detail layout in the Shattered Codex verdant theme: a socket rail lists every slot of the item (name, socketed gem, tint, and hidden state) so all slots can be edited in one window, with slot configuration in the middle column and Description/Condition editors in tabbed panes on the right. Edits are kept as drafts while switching slots and saved together with Save and Close.
- Socket Slot Settings now includes tint swatch shortcuts, a gem charge stepper with a capacity bar, an Add Socket button, an unsocket action on the gem row, and click-to-insert variable chips with explanatory tooltips under the condition editor. The empty Contents area also accepts dropping a gem to socket it directly from the window.
